<template>
  <el-dialog title="编辑文本" :visible.sync="show" width="600px" class="attention-text-wrap" :before-close="close">
    <div class="attention-text-content">
      ...
      <div class="text-content">
        <!-- @keyup="textInput(textarea,$event)"  -->
    ...
    <div class="add-replay-link" v-if="addReplayDialog">
      <el-dialog title="添加回复链接" width="585px" class="replay-link-dialog" :append-to-body="true" :visible.sync="addReplayDialog">
...
export default {
  ...
  methods: {
    ...
    removeLink() {
      ...
      if (this.form.linkStyle == 0) {
        // 文本链接
        this.xcxUrlObj.imageMediaId = '';
      } else if (this.form.linkStyle == 1) {
        // 小程序链接
    ...
    addItem() {
      ...
      if (sendValue.textNum > 300) {
        ...
        var reg = /<(?!(a|\/a|div|\/div|br)).*?>/gim; // 只保留a 和 div 标签
