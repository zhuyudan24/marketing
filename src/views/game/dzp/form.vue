  ...
  <el-form ref="form" :model="form" label-width="100px" :rules="rules" v-loading="loading" element-loading-text="拼命加载中">
    <section class="dm-form__wrap">
      ...
      <el-form-item label="banner图片" prop="gameImage">
        <!-- <img class="skin-box" v-if="form.skinType === 1" src="../../../assets/img/1.1.jpg" alt="" srcset="" />
      ...
      <el-form-item label="游戏说明" prop="gameIntroduction">
        <dm-input type="textarea" :rows="4" class="w400" v-model="form.gameIntroduction" :disabled="isInfo" :maxlength="200"></dm-input>
      </el-form-item>
    </section>
    <section class="dm-form__wrap">
      ...
      <el-form-item label="中奖几率" prop="gameWinningRate"> <el-input-number controls-position="right" v-model="form.gameWinningRate" :precision="2" :min="0" class="w400" :max="100" :disabled="isInfo"></el-input-number> % </el-form-item>
      <el-form-item label="限制抽奖" prop="gameLimitCount">
        <el-input-number controls-position="right" :min="-1" :precision="0" v-model="form.gameLimitCount" class="w400" :disabled="isInfo"></el-input-number> 次
        <p class="fz12 gray">
          <el-checkbox class="pl10" v-model="form.gameLimitDay" :precision="0" :disabled="isInfo" :true-label="1" :false-label="0">每天</el-checkbox>
          -1表示不限制抽奖次数
        </p>
      </el-form-item>
      <el-form-item label="免费抽奖" prop="gameFreeCount">
        <el-input-number controls-position="right" v-model="form.gameFreeCount" :min="0" :precision="0" class="w400" :disabled="isInfo"></el-input-number> 次
        <p class="fz12 gray">
      ...
      <el-form-item label="中奖机制">
        <ul>
          <li v-for="(v, i) in form.ruleList" :key="i" class="pb10">
            <p class="w200 inline-block">规则{{ i + 1 }}：会员第 {{ i + 1 }} 次抽奖获得</p>
            <el-select class="dm-select" size="medium" v-model="v.prizeSort" :disabled="isInfo" placeholder="选择发送状态">
              <el-option v-for="(v, i) in sortList" :key="i" :label="v.label" :value="v.value"></el-option>
            </el-select>
            <el-button type="text" v-if="i === form.ruleList.length - 1" @click="addRule(i)" :disabled="isInfo">新增规则</el-button>
            <el-button type="text" v-if="i !== 0" @click="delRule(i)" :disabled="isInfo">删除规则</el-button>
          </li>
          <div>
            <p class="w200 inline-block">结束规则：之后抽奖获得</p>
            <el-select class="dm-select" size="medium" v-model="endRuleObj.prizeSort" :disabled="isInfo" placeholder="选择发送状态">
              <el-option v-for="(v, i) in sortList" :key="i" :label="v.label" :value="v.value"></el-option>
            </el-select>
          </div>
        </ul>
      ...
      <el-form-item label="奖品设置" class="el-form-item__margin-top14">
        <el-table tooltipEffect="light" :data="tableList" style="width: 100%" row-click="key">
          ...
          <el-table-column prop="prizeType" label="奖品类型" align="left" :min-width="150">
            <template slot-scope="scope">
              <el-select size="small" v-model="scope.row.prizeType" class="width80" placeholder="选择奖品类型" :disabled="isInfo" @change="resetCount(scope.$index)">
                <el-option v-for="item in prizeReferTypeOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
              </el-select>
            </template>
          </el-table-column>
          <el-table-column label="奖项设置" align="left" :min-width="160">
            <template slot-scope="scope">
  ...
  data() {
    ...
    return {
      ...
      form: {
        ...
        shareFlag: 1,
        gameShareSettingId: '',
        shareTitle: '速玩大转盘，各种好礼等你盘',
        shareTextInfo: '大转盘来啦！你的“小幸运”转转转转起来，超值大奖等你拿回家！',
        shareImg: {
          code: '',
      ...
      banner: [
        ...
        { skinType: 1, code: '', imgUrl: require('../../../assets/img/1.1.jpg') }, // eslint-disable-line
        { skinType: 2, code: '', imgUrl: require('../../../assets/img/1.2.jpg') }, // eslint-disable-line
    ...
    async detailLottery() {
      ...
      try {
        ...
        this.form = Object.assign(this.form, {
          ...
          background: gameDetail.background, //4
          gameIntroduction: gameDetail.gameIntroduction, //5
          gameIntegral: gameDetail.gameIntegral || '', //6.1
          gameWinningRate: gameDetail.gameWinningRate || '', //2
          gameLimitCount: gameDetail.gameLimitCount || '', // 6.3 限制抽奖 次数
          gameLimitDay: gameDetail.gameLimitDay || '',
          gameFreeCount: gameDetail.gameFreeCount || '', // 6.4 免费抽奖
          gameFreeDay: gameDetail.gameFreeDay || '',
          ruleList: gameDetail.ruleList || [],
        ...
        this.form.ruleList.sort((a, b) => {
          return a.ruleSort - b.ruleSort;
        });
        if (this.form.ruleList.length > 1) {
          this.endRuleObj = this.form.ruleList.pop();
        }
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          let params = {
            ...
            gameLimitDay: this.form.gameLimitDay || 0, // 6.3 限制抽奖 每天的选择  1: 选择; 0: 不选
            gameFreeCount: this.form.gameFreeCount, // 6.4 免费抽奖次数 (免费次数<限制抽奖次数)
            gameFreeDay: this.form.gameFreeDay || 0, // 6.4 免费抽奖 每天的选择  1: 选择; 0: 不选
            gameStatus: 1,
          ...
          this.form.ruleList.forEach((v, i) => {
            ruleList.push({
              prizeSort: v.prizeSort == null ? 0 : v.prizeSort,
              ruleSort: i + 1,
              ruleStatus: 1
            });
          });
          ruleList.push({
            prizeSort: this.endRuleObj.prizeSort == null ? 0 : this.endRuleObj.prizeSort,
            ruleSort: this.form.ruleList.length + 1,
            ruleStatus: 1
          });
